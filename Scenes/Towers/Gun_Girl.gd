...
@onready var anim2 = $Node2D/BulletHit
@onready var anim3 = $Node2D/BulletHit2
@onready var anim4 = $Node2D/BulletHit3
...
func _physics_process(delta):
	...
	if enemy_array.size() !=0:
		...
		$Node2D/BulletHit.position = to_local(enemy.position) + Vector2(-5,-5)
		$Node2D/BulletHit2.position = to_local(enemy.position) + Vector2(5,0)
		$Node2D/BulletHit3.position = to_local(enemy.position) + Vector2(5,15)
		...
		if red:
			...
			anim2.play("bullet_onhit")
			await(get_tree().create_timer(.1).timeout)
			anim3.play("bullet_onhit")
			await(get_tree().create_timer(.1).timeout)
			anim4.play("bullet_onhit")
			shoot()
		...
		$machinegun.playing = false
		$Node2D/BulletHit.visible = false
		$Node2D/BulletHit2.visible = false
		$Node2D/BulletHit3.visible = false
...
func shoot():	
			...
			$Node2D/BulletHit.visible = true
			$Node2D/BulletHit2.visible = true
			$Node2D/BulletHit3.visible = true
